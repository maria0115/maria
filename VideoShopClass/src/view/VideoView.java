...
//				}
		...
		tableVideo.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				try {
					...
					String direc = vo.getDirector();
					String exp = vo.getExp();
					String genre = vo.getGenre();
					String title = vo.getVideoName();
					int nom = vo.getVideoNo();
					
					tfVideoTitle.setText(title); 
					tfVideoDirector.setText(direc);
					tfVideoActor.setText(actor);
					taVideoContent.setText(exp);
					tfVideoNum.setText(Integer.toString(nom));

					comVideoJanre.setSelectedItem(genre);
				} catch (Exception e1) {
					System.out.println("검색 입력 실패"+e1.getMessage());
				}
	...
	public void registVideo(){
		...
		String videodirec = tfVideoDirector.getText();
		String videoactor = tfVideoActor.getText();
		String videocontent = taVideoContent.getText();
		
		videocount = Integer.parseInt(tfInsertCount.getText());
		String janre = (String)comVideoJanre.getSelectedItem();
		//2. video 클래스의 멤버 로 저장하기
		vo = new Video();

		vo.setActor(videoactor);
		vo.setDirector(videodirec);
		vo.setExp(videocontent);
		vo.setGenre(janre);
		vo.setVideoName(videoTitle);
		
		

		//3. 모델단(VideoModel)의 insertVideo()를 호출하여 2번의 객체 넘기기
		try {
			model.insertVideo(vo, videocount);
		} catch (Exception e) {
			System.out.println("vo에 잘 못들어 갔다"+e.getMessage());
		}
		//4. 화면 초기화
	...
	public void modifyVideo(){
		...
		//1. 각 화면의 입력값들 얻어오기
				...
				//3. 모델단(VideoModel)의 insertVideo()를 호출하여 2번의 객체 넘기기
				try {
					model.updateVideo(vo, videocount);
				} catch (Exception e) {
					System.out.println("vo에 잘 못들어 갔다"+e.getMessage());
				}
				//4. 화면 초기화
	...
	// 비디오현황검색
		public void searchVideo(){
			...
			int type = (int)comVideoSearch.getSelectedIndex();
			String word = tfVideoSearch.getText();
			
			try {
				ArrayList data = model.selectVideo(type, word);
