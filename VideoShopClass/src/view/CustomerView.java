...
public class CustomerView extends JPanel 
{
	...
	public void registCustomer(){
		...
		vo.setCustAddr(custAddr);
		vo.setCustEmail(custEmail);
		vo.setCustName(custName);
		vo.setCustTel1(custTel1);
		vo.setCustTel2(custTel2);
		
		try {
			model.insertCustomer(vo);
		} catch (Exception e) {
			System.out.println("텍스트 필드 값 입력 실패"+e.getMessage());
		}
	...
	public void searchByTel(){
		...
		try {
			...
			System.out.println("전화번호 입력 실패 : "+e.getMessage());
		}	
	...
	public void updateCustomer(){
		...
		try {
			...
			String custTel1 = tfCustTel.getText();
			String custTel2 = tfCustTelAid.getText();
			String CustTelSearch = tfCustTelSearch.getText();
			...
			vo.setCustTel1(custTel1);
			vo.setCustTel2(custTel2);
			vo.setTfCustTelSearch(CustTelSearch);
